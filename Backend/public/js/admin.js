document.addEventListener("DOMContentLoaded", function () {
    ...
    for (var i = 0; i < navLinks.length; i++) {
        ...
        if (currentPath === linkPath || (menuName && currentPath.includes(menuName))) {
            ...
            if (this.files && this.files.length > 0) {
                for (let i = 0; i < this.files.length; i++) {
                    ...
                    reader.onload = function (e) {
                        ...
                        let image = $('<img class="coverInput" src="' + e.target.result + '">');
                        let deleteIcon = $('<i id="delete-cover-icon" class="fas fa-times delete-icon"></i>');
...
$(document).ready(function () {
    ...
    function updateSelectedCheckboxes() {
        selectedCheckboxes = $('.form-check-input:checked').map(function () {
            return $(this).attr('id');
        }).get();
    }
    ...
    $('#firstHeadingDownload, #companyHeadingDownload').on('input', storeModalInputs);
...
async function fetchUnreadCounters() {
    ...
    document.getElementById('contactUsCounter').textContent = jsonResponse.data.contactUsCount;
    document.getElementById('orderCounter').textContent = jsonResponse.data.orderCounter;
